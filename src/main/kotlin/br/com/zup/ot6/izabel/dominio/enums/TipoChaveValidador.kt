...
enum class TipoChaveValidador(val descricao: String) {
    CPF("CPF") {
        override fun isValida(chave: String): Boolean {
            ...
            throw IllegalArgumentException(MENSAGEM_DE_ERRO.format(descricao, "12345678901"))
        }
    },
    CHAVE_ALEATORIA("CHAVE_ALEATORIA") {
        override fun isValida(chave: String): Boolean {
            if(chave.isNullOrEmpty()) return true
            throw IllegalArgumentException(MENSAGEM_DE_ERRO.format(CHAVE_ALEATORIA.descricao, "STRING VAZIA"))
        }
    },
    EMAIL("EMAIL") {
        override fun isValida(chave: String): Boolean {
            ...
            throw IllegalArgumentException(MENSAGEM_DE_ERRO.format(EMAIL.descricao, "exemplo@email.com"))
        }
    },
    TELEFONE("TELEFONE") {
        override fun isValida(chave: String): Boolean {
            if(isTamanhoValido(chave) && chave.matches("^\\+[0-9][0-9]\\d{1,14}\$".toRegex())) return true
            throw IllegalArgumentException(MENSAGEM_DE_ERRO.format(TELEFONE.descricao, "+5585988714077"))
        }
    };
